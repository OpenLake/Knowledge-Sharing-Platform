    ...
    refetchNotes
        ...
        const updateNoteHandler = (e: any) => {
            if (title === "" || selectedSubjectCode === "" || selectedSubjectName === "" || selectedBatch === "" || selectedBranch === "" || url === "")
                ...
                updateNote(selectedNote.id, title, selectedSubjectCode, selectedBatch, selectedClass, selectedBranch, url, isAnonymous, refetchNotes)
                setTitle('')
                setUrl('')
                setSelectedBatch('')
                setSelectedBranch('')
        ...
        useEffect(() => {
            ...
            setSelectedBatch(selectedNote.batch)
            setSelectedBranch(selectedNote.branch)
            setSelectedClass(selectedNote.class)
            setSelectedSubjectCode(selectedNote.subject_code)
            setSelectedSubjectName(selectedNote.subject.name)
            setIsAnonymous(selectedNote.anonymous)
        }, [selectedNote])
        ...
        return (
            selectedNote ?
                <div className={`${!selectedNote && 'hidden'} flex justify-center items-center fixed top-0 left-0 right-0 z-50 w-full bg-black/50 overflow-x-hidden overflow-y-auto h-full`}>
                    <div className="relative w-full pt-32 md:pt-48 px-5 h-full max-w-2xl md:h-auto">
                        <div ref={addNoteModalRef} className={`relative bg-white rounded-lg shadow`}>
                            ...
                            <div className="flex flex-col p-10 space-y-2">
                                ...
                                <div className="flex flex-col space-y-1 relative">
                                    ...
                                    <label htmlFor="subjectCode" className="flex items-center">
                                        <input
                                            ...
                                            onFocus={() => subjects.length ? setShowSubjectCodeDropdown(true) : setShowSubjectCodeDropdown(false)}
                                            onChange={(e) => {
                                ...
                                <div className="flex flex-col space-y-1 relative">
                                    ...
                                    <label htmlFor="subjectName" className="flex items-center">
                                        <input
                                            ...
                                            onFocus={() => subjects.length ? setShowSubjectNameDropdown(true) : setShowSubjectNameDropdown(false)}
                                            onChange={(e) => {
                                ...
                                <button disabled={isLoading} onClick={updateNoteHandler} type="submit" className="flex items-center w-fit space-x-2 p-2 duration-200 transition-all rounded-md shadow-md hover:shadow-xl disabled:cursor-not-allowed disabled:bg-primary/70 bg-primary text-white font-semibold">
                                    <p>
                                        Update Notes
                                    </p>
                                </button>
